...
export const Amigos = (props) => {
  ...
  const traerAmigos = async () => {
    try {
      ...
      const peticiones = await SolicitudService.getSolicitudesPendientes(
        params.id
      );
  ...
  return (
    <View style={[
      ...
      <View style={styles.solicitudesPendientes}>
        <BotonFlotante
          onPress={handleVerSolicitudesPendientes}
          name="email"
          style={styles.botonFlotante}
        />
        {solicitudes.length > 0 && (
          <View style={styles.burbujaSolicitudes}>
            <Text
              style={[
                styles.burbujaSolicitudesTexto,
                solicitudes.length > 9 && styles.textoLargo,
              ]}
            >
              {solicitudes.length}
            </Text>
